...
//    @Modifying
//    @Query("DELETE FROM Major as m where m.name = :name")
//    int deleteByName(@Param(value = "name") String majorName);
    ...
    @Query(value = "SELECT distinct m from Major as m left join fetch m.students as stu left join fetch stu.projects where m.id = :id")
